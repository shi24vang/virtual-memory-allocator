/* Allocator overview
 * - List-based fits share a 4 KiB arena. Free blocks are kept in address order
 *   for cheap coalescing and indexed by a skip list for best/worst-fit lookups.
 * - Next-fit keeps a rover so scans resume where the previous search ended.
 * - Buddy allocator uses its own 4 KiB arena backed by power-of-two free lists.
 * - malloc_* return NULL on size==0 or when no suitable block exists.
 * - my_free ignores invalid pointers or double frees silently.
 * - When a split would leave an unusable tail (< header + MIN_TAIL), the entire
 *   block is returned to the caller instead of creating tiny fragments.
 * - All metadata lives inside the arenas; no calls back into the C runtime.
 * - Skip list heights come from a small fixed-seed XOR-shift PRNG to keep runs
 *   reproducible.
 */
#include "allocator.h"
...
// Debug hooks are no-ops unless MMU_DEBUG is defined.
#ifdef MMU_DEBUG
  #include <assert.h>
 ...
 * This header sits right before the user payload. Each block participates in:
 *   1) Address list:  aprev <-> this <-> anext   (merges with neighbors)
...
typedef struct free_blk {
    ...
    struct free_blk *anext;          // address-ordered list (next/prev)
 ...
 * - If the free list becomes empty, clear rover to avoid dangling pointers.
 */
...
void my_free(void *ptr){
    if (!ptr) return;
    // Buddy pointers live in a separate arena.
    if (b_inited){
        uintptr_t p  = (uintptr_t)ptr;
        uintptr_t b0 = (uintptr_t)b_arena, b1 = b0 + HEAP_SIZE;
        if (p >= b0 && p < b1){
            bud_t *b = (bud_t*)((char*)ptr - BUDHDR);
            if (b->magic != MAGIC_A) return;  // silently ignore invalid frees
            bfm(b);
            return;
        }
    }
    ...
    if (blk->magic != MAGIC_A) return;       // silently ignore invalid frees
    // Insert by address
    if (!alist_head){
